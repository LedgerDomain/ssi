...
impl Credential {
    ...
    pub async fn decode_verify_jwt(
        ...
        let keys = match header.key_id {
            ...
            Some(kid) => {
                let key = match crate::ldp::resolve_key(&kid, resolver).await {
                    Ok(key) => key,
                    Err(err) => {
                        return (
                            None,
            ...
            None => {
                ...
                let issuer = match claims.issuer {
                    ...
                    None => {
                        return (
                            None,
                            VerificationResult::error("\"iss\" field missing"),
                        );
                    }
                };
//                 println!("Credential::decode_verify_jwt; issuer: {:?}", issuer);
                ...
                let vmms =
                    match crate::did_resolve::get_verification_methods(
                        &issuer,
                        proof_purpose,
                        resolver,
                    ).await {
                        ...
                        Err(err) => {
                            return (
                                None,
                                VerificationResult::error(&format!("failed to get verification methods for DID {}; {}", issuer, err)),
                            );
                        },
                    };
                ...
                for (vmm_id, vmm) in vmms {
                    let key = match vmm.get_jwk() {
                        ...
                        Err(err) => {
                            return (
                                None,
                                VerificationResult::error(&format!("failed to get JWK from verification method {}; {}", vmm_id, err)),
                            );
                        }
                    };
                    keys.push(key);
...
//                 println!("Credential::decode_verify_jwt; returning using key: {:?}", key);
        ...
        {
            results.errors.push("Unable to filter proofs".to_string());
